...
export async function GET(request: Request) {
  ...
  const upstreamUrl = new URL('/runtime/stream', API_BASE_URL);
  ...
  try {
    ...
    if (!upstreamResponse.ok || !upstreamResponse.body) {
      let payload: ErrorBody = { error: 'Runtime stream unavailable.' };
      try {
    ...
    return NextResponse.json({ error: 'Runtime stream unavailable.' } satisfies ErrorBody, { status: 502 });
  } finally {
